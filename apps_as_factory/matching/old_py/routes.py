...
def match_row_to_gazetteer(input_row, gazetteer_df, patron_saints):
    ...
    spatial_fields = [
        "ref_Adm2", "ref_Partido_generico", "ref_Partido", "ref_Jurisdiccion",
        "ref_Provincia_menor", "ref_Provincia", "ref_Provincia_mayor", "ref_Adm1",
        "ref_Obispado", "ref_Adm0", "ref_Adm0_ISO", "ref_Audiencia", "ref_Region"
    ]
    ...
    for field in ["ref_Partido_generico", "ref_Provincia_generica", "ref_Region", "ref_Adm0_ISO"]:
    ...
    if not results:
        ...
        # ============================================================
        # NEW: MODERN ADMIN BOUNDARY FALLBACK
        # ============================================================
        logger.info(f"[{row_id}] Attempting modern admin boundary fallback...")
        
        modern_admin_fields = [
            ("ref_Adm0_ISO", "lugar_adm0_iso"),
            ("ref_Adm0", "lugar_adm0"),
            ("ref_Adm1", "lugar_adm1"),
            ("ref_Adm2", "lugar_adm2"),
        ...
        fallback_df = gazetteer_df.copy()
        matches_found = False
        
        # Try to filter by modern admin fields
        for ref_field, gz_field in modern_admin_fields:
            ref_value = input_row.get(ref_field)
            
            if pd.isna(ref_value) or str(ref_value).strip() == "":
                continue
            
            # Convert region code to name if needed
            if ref_field == "ref_Region" and ref_value in REGION_CODE_TO_NAME:
                ref_value = REGION_CODE_TO_NAME[ref_value]
            
            if gz_field not in fallback_df.columns:
                logger.debug(f"[{row_id}] Column {gz_field} not in gazetteer, skipping")
                continue
            
            cleaned_ref = clean_toponym(str(ref_value))
            
            # Try exact match first
            exact_mask = fallback_df[gz_field].fillna("").apply(
                lambda x: clean_toponym(str(x)) == cleaned_ref
            )
            
            if exact_mask.any():
                fallback_df = fallback_df[exact_mask]
                matches_found = True
                logger.info(f"[{row_id}] Modern admin exact match on {ref_field}: {len(fallback_df)} rows")
                continue
            
            # Try fuzzy match (threshold 0.85)
            fuzzy_indices = []
            for idx, row_data in fallback_df.iterrows():
                gz_value = row_data.get(gz_field)
                if pd.notna(gz_value) and str(gz_value).strip() != "":
                    cleaned_gz = clean_toponym(str(gz_value))
                    if cleaned_gz:
                        score = fuzz.ratio(cleaned_ref, cleaned_gz) / 100
                        if score >= 0.85:
                            fuzzy_indices.append(idx)
            
            if fuzzy_indices:
                fallback_df = fallback_df.loc[fuzzy_indices]
                matches_found = True
                logger.info(f"[{row_id}] Modern admin fuzzy match on {ref_field}: {len(fallback_df)} rows")
        
        # If we found matches using modern boundaries, try toponym matching
        if matches_found and not fallback_df.empty:
            logger.info(f"[{row_id}] Found {len(fallback_df)} candidates via modern admin boundaries")
            ...
            if year:
                year = min(max(year, 1701), 1808)
                if 'overlap_start' in fallback_df.columns and 'overlap_end' in fallback_df.columns:
                    fallback_df = fallback_df[
                        (fallback_df['overlap_start'] <= year) & 
                        (fallback_df['overlap_end'] >= year)
                    ]
                    logger.debug(f"[{row_id}] After time filter: {len(fallback_df)} rows")
            
            if not fallback_df.empty:
                fallback_results = match_toponyms_tiered(input_row, fallback_df, patron_saints)
                
                if fallback_results and fallback_results[0].get("toponym-match") != "null":
                    # Mark these as candidates (not auto_adopt) since they came from fallback
                    for result in fallback_results:
                        if result.get("phase-1-outcome") == "auto_adopt":
                            result["phase-1-outcome"] = "candidate"
                        result["territories-match"] = "modern_admin_fallback"
                    
                    logger.info(f"[{row_id}] Modern admin fallback found {len(fallback_results)} results")
...
def run_matching():
    ...
    territory_ref_candidates = [
        "ref_Adm0_ISO", "ref_Adm0", "ref_Region", "ref_Audiencia",
...
def disambiguate(prefix):
    ...
    try:
        ...
        gazetteer_cols = ["gz_id", "lugar_categoria", "lugar_categoria_especial", 
                         ...
                         "lugar_provincia_generica", "lugar_region", "lugar_Adm0_ISO", "lugar_Adm0", 
                          "Nivel"]
