...
DECISION_TREE = {
    ...
    '1.2.1.1': {
        ...
        'actions': [
            {'text': 'Download Guidebook: "Prepare data from scratch"', 'link': '/guidebook/3'},
    ...
    '1.2.2': {
        ...
        'actions': [
            {'text': 'Download Guidebook: "Compile lists using search"', 'link': '/guidebook/4'},
...
• To understand for which places you may need to compile data. E.g., you want to study tithes in a certain province. You pull a list for all parishes in that province in order to find for which places you should get data.''',
    ...
    '1.3': {
        ...
        'actions': [
            {'text': 'Download Guidebook: "Improve HGIS-Indias"', 'link': '/guidebook/2'},
    ...
    '2.2.1': {
        ...
        'actions': [
            {'text': 'Download Guidebook: "Upload and process file"', 'link': '/guidebook/1'},
...
@orientation_bp.route('/')
def index():
    session.clear()
    return redirect(url_for('step', step_id='start'))

@orientation_bp.route('/step/<step_id>')
def step(step_id):
...
@orientation_bp.route('/back/<step_id>')
...
@orientation_bp.route('/upload-processor')
...
@orientation_bp.route('/api/upload', methods=['POST'])
def api_upload():
...
@orientation_bp.route('/api/process_domain', methods=['POST'])
...
@orientation_bp.route('/api/save_column_types', methods=['POST'])
...
@orientation_bp.route('/api/get_field_mapping_info', methods=['GET'])
...
@orientation_bp.route('/api/process_final', methods=['POST'])
...
@orientation_bp.route('/api/download/<filename>')
...
@orientation_bp.route('/guidebook/<int:number>')
...
@orientation_bp.route('/template/<template_type>')
