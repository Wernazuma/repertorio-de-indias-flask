...
log_file_path = os.path.join(log_folder, "matching_phase_1.log")
...
def match_with_progress():
    ...
    result_file = os.path.join(UPLOAD_FOLDER, f"{prefix}_phase_1.csv")
    
    # Check if matching is already complete
    matching_complete = os.path.exists(result_file)
    
    # Check if matching is already in progress
    matching_in_progress = os.path.exists(status_file) and not matching_complete
    
    # If matching hasn't started yet, start it in background
    if not matching_in_progress and not matching_complete:
        try:
            # Validate input files exist before starting
            input_path = os.path.join(UPLOAD_FOLDER, f"{prefix}_cleaned.csv")
            if not os.path.exists(input_path):
                return f"Input file not found: {prefix}_cleaned.csv", 404
            if not os.path.exists(GAZETTEER_FILE):
                return "Gazetteer file not found", 500
            
            # Start matching in background thread
            thread = threading.Thread(target=run_matching_background, args=(prefix,))
            thread.daemon = True
            thread.start()
            
            # Give the thread a moment to start and create initial status
            time.sleep(0.1)
            flash("Matching started...")
            
        except Exception as e:
            return f"Error starting matching: {e}", 500
    ...
    try:
        ...
        content = "Starting matching process..."
    
    # Re-check completion status after potential background start
    matching_complete = os.path.exists(result_file)
    
    return render_template("phase_1_matching.html", 
                         prefix=prefix, 
                         content=content, 
                         matching_complete=matching_complete)


def run_matching_background(prefix):
    status_file = os.path.join(UPLOAD_FOLDER, f"{prefix}_status.txt")
    try:
        input_path = os.path.join(UPLOAD_FOLDER, f"{prefix}_cleaned.csv")
        ...
        with open(status_file, "w", encoding="utf-8") as f:
            f.write(f"Starting to match {total_rows} rows...\n")
        ...
        for i, row in input_df.iterrows():
            ...
            # Update status every row
            with open(status_file, "w", encoding="utf-8") as f:
                progress_pct = ((i + 1) / total_rows) * 100
                f.write(f"Matching row {i + 1} of {total_rows} ({progress_pct:.1f}%)\n")
        ...
        result_path = os.path.join(UPLOAD_FOLDER, f"{prefix}_phase_1.csv")
        result_df.to_csv(result_path, sep=';', index=False)
        
        # Final status update
        with open(status_file, "w", encoding="utf-8") as f:
            f.write(f"Matching complete! Processed {total_rows} rows.\n")
        ...
        with open(status_file, "w", encoding="utf-8") as f:
            f.write(f"Error during matching: {e}\n\nTraceback:\n{tb}")
        logger.error(f"Matching error for prefix {prefix}: {e}\n{tb}")
...
def disambiguate(prefix):
    input_path = os.path.join(UPLOAD_FOLDER, f"{prefix}_phase_1.csv")
    if not os.path.exists(input_path):
...
def set_constraints(prefix):
    ...
    region_choices = df["Region"].dropna().unique().tolist()
    region_choices.sort()
    ...
    if request.method == "POST":
        ...
        flash("Constraints saved. You can now run matching.")
        return redirect(url_for("upload_form", prefix=prefix))
