...
def run_matching():
    ...
    # Normalize gz_id column if present
    if "gz_id" in result_df.columns:
        result_df["gz_id"] = pd.to_numeric(result_df["gz_id"], errors="coerce")
    ...
    # Drop ref_Label from the input context to avoid _x/_y columns
    ref_columns_no_label = [c for c in ref_columns if c != "ref_Label"]
    input_context = input_df[["rowID"] + ref_columns_no_label].copy()

    processing_df = result_df.merge(input_context, on="rowID", how="left", sort=False)
...
def disambiguate(prefix):
    ...
    try:
        ...
        if "ref_Label_x" in merged_df.columns:
            merged_df.rename(columns={"ref_Label_x": "ref_Label"}, inplace=True)
        if "ref_Label_y" in merged_df.columns:
            merged_df.drop(columns=["ref_Label_y"], inplace=True)
